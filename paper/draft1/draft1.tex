...
The APOGEE survey \citep{Majewski:2015} of the Sloan Digital Sky Survey IV (SDSS-IV) is a high resolution (R$\sim$22,500), H-band (1.5-1.7$\mu$m) survey which has observed over 260,000 stellar spectra since its fourteenth data release (DR14; \citealt{Abolfathi:2017}). Fundamental parameters for each of these stars are estimated by the APOGEE Stellar Parameter and Chemical Abundances Pipeline (ASPCAP; \citealt{Perez:2016}), which employs a $\chi^2$ fitting procedure to fit ATLAS9 radiative transfer models to determine atmospheric parameters (temperature, surface gravity, metallicity) and 15 chemical abundances \citep{Meszaros:2012}. 

APOGEE is primarily targeted for bright stellar populations, with de-reddened photometry and color cutoffs of $7 \leq H \leq 13.8$ and $[J-K]_0 \geq 0.5$ \citep{Zasowski:2013} with the objective of studying galactic composition and evolution. However numerous (specific estimate based on Gaia?) cool, main sequence sources have also been observed either accidentally, or as targets proposed by the M dwarf ancilliary survey \citep{Desphande:2013}.


\subsection{Training Samples}

The Cannon uses a \emph{data-driven} approach for constructing a model which parameterizes the flux at each pixel of a spectrum in terms of a set of stellar labels, a flexible number of parameters chosen by the user. The basic principle 

For the purpose of this study we have constructed two different training samples: first a one-dimensional \emph{spectral type model}, and second a two-dimensional \emph{physical parameter model} (the choice of training labels and dimensionality of our data set we discuss further in Section \ref{sec:discussion}).

The spectral type sample consists of 51 sources from \citealt{West:2011}, spanning M0-M9 
...
Constructing such a model requires two steps: first a \emph{training step} in which a polynomial model is 
