...
\thecannon\ model can in principle be trained on any physical or empirical labels available \color{red}{\bf beyond those which typically parameterize theoretical atmospheric models ($\teff$, logg, [M/H], etc.), such as additional physical parameters (e.g., mass/age \citealt{Ness:2016}) or empirical proxies for physical parameters (e.g., spectral types, colors, magnitudes), }\color{black} giving a wide range of flexibility to the model. 
...
Finally, \thecannon\ assumes that the dependence of \color{red}{\bf the }\color{black} spectrum on labels is continuous and smooth---and in this implementation, well approximated by quadratic functions. If that isn't true, there will be features that \thecannon\ cannot reproduce.
...
\color{red}{\bf however, they }\color{black} are much higher.
...
\color{red}{\bf We also note that this analysis was completed using data from Data Release 14 of APOGEE, 
which did include molecular lines from FeH in the pipeline at the time, which become numerous and strong for $\teff \lesssim 3600$\,K. }\color{black}
Further analysis would need to be done to \color{red}{\bf quantify the metallicity improvement for M dwarfs in later data releases of APOGEE now available (in which FeH has been included in analysis)}\color{black}, and determine if the metallicity bias
