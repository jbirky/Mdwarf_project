...
with their properties and compositions.
However, insufficient physical assumptions behind atmospheric models
have limited abilities at reproducing spectral features in stars of the 
coolest temperature regimes ($T_{\rm eff} < 4200\,$K) and fully
exploiting the information content and statistical extent of
current and upcoming large-scale spectroscopic surveys.
Here we present a catalog of spectroscopic temperatures, metallicities,
spectral types for 5,875 M dwarfs in the \apogee\ survey using
...
$2860 < \teff < 4130$\,K callibrated with bolometric temperatures, 
$-0.5 < \feh < 0.5$\,dex callibrated with FGK wide binary metallicities, 
...
with known SDSS optical spectral types ranging M0 to M9, to predictive accuracy of
0.9 types.
\color{gcolor}{From the overlap of \textsl{APOGEE DR14} and \textsl{Gaia DR2} we
apply our models and compare our \cannon\ derived $\teff$ against color--temperature
relations and \cannon\ derived $\feh$ against isochrone models in
\gaia\ color-magnitude space.
Finally we use our results to
estimate stellar radii from absolute magnitudes and \cannon\ temperatures [AND WE FIND WHAT??].}\color{black}
\end{abstract}
...
%JB: I've swapped some sentences from section 2.2 and section 3, which explain the terminology. Do you think this paragraph adds enough clarity now without being too redundant with the following sections 2 & 3 describing The Cannon?
...
% In each case, \thecannon\ produces a predictive model for \apogee\ spectra, but without any input of stellar physics beyond whatever went in to producing the labels in the first place. That is, the model to be applied to the \apogee\ data is learned from the \apogee\ data alone. This obviates physical models that are often inaccurate for M dwarfs, and capitalizes on the excellent data quality delivered by the \apogee\ spectrograph and project. 
%JB: I think this part is redundant
...
\color{gcolor}{{In Section \ref{sec:results} we present our experimental results, evaluate the predictive accuracy of our models, apply our model to a selected test sample of $>$10,000 sources, and examine the validity of our parameters against color-temperature relations and isochrone grids.
Finally, in Section \ref{sec:discussion} we discuss model performance, future improvements, and implications of our results in the context of present and upcoming stellar and exoplanetary surveys.}\color{black} \\
...
\begin{figure*}
\begin{center}
	\includegraphics[width=.48\linewidth]{figures/ruwe_cut_before.png}
	\includegraphics[width=.48\linewidth]{figures/ruwe_cut_after.png}
	\caption{}
	\label{fig:ruwe_cut}
\end{center}
\end{figure*}
...
\item \textbf{Model extraploation cuts:} Because extrapolation with \thecannon\ is unreliable, we select only sources with labels inside the range of the training sample: for the \cannon\ reported temperatures and metallicities we impose $2850<T_{\rm eff}<4150$ and $-0.5<\feh<0.5$, and for \cannon\ reported spectral subtypes we impose $0<SPT<9$.

\item \textbf{Astrometric cut:} \gaia\ re-normalised unit weight error (RUWE) $RUWE<1.2$

\item \textbf{Binary cut:}
...
% Comparisons between the West-trained spectral types and Mann-trained temperatures and metallicities are shown in Figure \ref{fig:west_vs_mann}: left shows the slight negative correlation between spectral type and $\feh$, and right shows the nonlinear decrease in temperature with visible metallicity gradient. 
We also note that applying our model requires very little computational demand: the time to train and test a model on all 14,827 sources was two minutes on a personal laptop. 
...
While the large bulk of points falling within the temperature ranges of the plots shown does not qualitatively show a lot more deviation from color-temperature relations, the average ASPCAP temperatures are much hotter, and scatters are much higher skewed by 500 sources which ASPCAP predicts to have temperatures over 4200K.}\color{black}
...
% 	\caption{Comparison of \cannon\ $\feh$ as compared to metallicity predicted by PARSEC isochrone tracks \citep{Bressan:2012} in \gaia\ color-magnitude space. The two panels show five metallicity tracks (ranging $-0.5<$[M/H]$<+0.5$ in steps of 0.1 dex) at two different ages (3 and 7 Gyr). In the ASPCAP plot only the 8,960 out of 5,875 sources with metallcities $-0.5<$[M/H]$<+0.5$ are shown, while the remaining 1,351 sources with [M/H]$<-0.5$ and [M/H]$>+0.5$.}
% 	\label{fig:isochrone_cmd}
% \end{figure}

% \begin{figure*}
% 	\plotone{figures/aspcap_cannon_label_hist.png}
% 	\caption{Distribution of temperatures and metallicities for 5,875 sources from \thecannon\ compared to the ASPCAP pipeline. Note that \cannon\ temperatures only span $2850<T_{\rm eff}<4150$ and $-0.5<\feh<0.5$\,dex because we have imposed cutoffs for stars that fall outside of the training sample range, however for the same 5,875 stars ASPCAP predictions are skewed higher in temperature and lower in metallicity compared to \cannon\ measurements.}
...
\begin{center}
	...
	\includegraphics[width=.49\linewidth]{figures/cmd_teff_safe.png} \\
	\includegraphics[width=.45\linewidth]{figures/cmd_feh_full.png}
	\includegraphics[width=.49\linewidth]{figures/cmd_feh_safe.png} \\
	\includegraphics[width=.45\linewidth]{figures/cmd_spt_full.png}
	\includegraphics[width=.49\linewidth]{figures/cmd_spt_safe.png}
...
\begin{figure*}[]
\begin{center}
	\includegraphics[width=5cm]{figures/met_comp1.png}
	\includegraphics[width=5cm]{figures/met_comp2.png}
	\includegraphics[width=5cm]{figures/met_comp3.png} 
	\includegraphics[width=5cm]{figures/met_comp4.png}
	\includegraphics[width=5cm]{figures/met_comp5.png}
	\includegraphics[width=5cm]{figures/met_comp6.png} 
	\includegraphics[width=5cm]{figures/met_comp7.png}
	\includegraphics[width=5cm]{figures/met_comp8.png}
	\includegraphics[width=5cm]{figures/met_comp9.png} 
...
% 	\caption{Comparison of radii estimated from M15 relations: on the x-axis radii are derived from  K magnitudes and \cannon\ $\feh$, on the y-axis radii are derived from \cannon\ $\teff$ and $\feh$.}
% 	\label{fig:radii}
% \end{figure*}
